...
function itemLib.formatValue(value, baseValueScalar, valueScalar, precision, displayPrecision, ifRequired)
	...
	elseif displayPrecision then
		...
		local displayVal = roundSymmetric(value, precision and m_min(2, m_floor(math.log(precision, 10) + 0.001)) or 2) -- max decimals ingame is 2
		local intVal = math.tointeger(displayVal)
		return tostring(intVal ~= nil and intVal or displayVal)
	end
end
