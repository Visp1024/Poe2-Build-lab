		...
		if not isAttack then
			...
			if breakdown then
				...
				for _, entry in ipairs(hitChances) do
					...
					-- Lua 5.4 (NLua host): floats break string.len/buffer indexing; force integers.
					local cappedInt = m_floor(entry.capped)
					entry.cappedBuffer = buffers.chance[string.len(cappedInt)] or ""
					local excessInt = entry.excess and m_floor(entry.excess)
					entry.excessText = excessInt and " ^8(+" .. (buffers.chance[string.len(excessInt)+1] or "") .. excessInt .. "%)" or ""
					entry.config = (entry.distance == enemyDistance) and " ^8(current config)" or ""
					t_insert(breakdown.AccuracyHitChance, entry.distBuffer .. entry.distance .. "m: " .. entry.cappedBuffer .. cappedInt .. "%" .. entry.excessText .. entry.config)
				end
