...
- [x] **Личные пути** — `D:\Projects\…` и `C:\Users\…` в трекнутых файлах не
      встречаются (чинилось в задачах #29, #32 и #39). Аудит #36 объявил
      этот пункт чистым преждевременно: в `PBLExport/ggpk_export/inspect.mjs`
      оставался путь `C:/Users/<пользователь>/…` — одноразовый отладочный
      скрипт удалён в #39 и вырезан из истории.
- [x] **Подписи коммитов** — история `main` переписана в #39: автор и коммиттер
      всех коммитов приведены к `visp1024@users.noreply.github.com`, рабочий
      email в репозитории не встречается. В репозитории задан локальный
      `user.email`, чтобы новые коммиты не подхватывали глобальный.
