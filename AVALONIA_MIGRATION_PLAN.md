...
### Passive tree
- [ ] **Tree version conversion (0_4 → 0_5)** — версия дерева хранится per-spec в build XML (`Spec treeVersion=`); старые билды остаются на 0_4 и не видят новые восхождения (Spirit Walker, Martial Artist). Оригинальный PoB показывает кнопку конвертации, когда `spec.treeVersion ~= latestTreeVersion` (`PassiveSpec.lua:323` выставляет `showConvert`, конвертация — `TreeTab.lua` `ConvertToVersion`). В нашем UI нужно: баннер/кнопка «Конвертировать в 0_5» на TreeTab при устаревшем спеке, вызов соответствующего Lua-пути, refresh дерева и статов. Учесть: при конвертации часть нод может пропасть (upstream удаляет/двигает ноды) — показать diff/предупреждение.
- [x] **Path preview on hover** — done: `TreeCanvas` BFS from the allocated set to the hovered unallocated node, dashed orange route + rings on intermediate nodes, cached per (hovered node, alloc set). Commit `1f01b3c7f`.
- [x] **Async optimistic alloc** — done: `TreeTabViewModel` predicts the alloc path C#-side (BFS), updates the allocated set optimistically + repaints, then runs `spec:AllocNode/DeallocNode` + recalc on a background thread serialized by a `SemaphoreSlim`; hover skips its Lua call while a toggle is in flight; real state reconciled from Lua when the burst drains. Per-click UI cost ~73 ms → ~0.3 ms. Commit `7299519b3`.
