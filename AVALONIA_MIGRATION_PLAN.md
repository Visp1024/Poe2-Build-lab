...
- [ ] HiDPI / DPI scaling
- [x] Dark / light theme — configurable Dark / Light / System (follows Windows), instant apply, persisted in prefs (`AppTheme`), default Dark. `ThemeService` (PBLApp) + `IThemeSwitcher`/`AppSettingsViewModel` (Core) + `AppSettingsWindow` (gear button in BuildList header). Light token palette already existed in `Tokens.Colors.axaml`; visible-screen hardcode swept to tokens. Tree canvas + item/hover tooltips deliberately stay game-dark in both variants. Deferred: Trader hardcode (TODO in XAML), tree-canvas light palette, user color customization.
- [ ] GitHub Actions CI
...
### Phase 17 — Items overhaul: main-skill header, hover/right-pane styling, editor polish ✅ DONE

**Unified main-skill selector + smart CastOn default** (`BuildPageView.axaml`, `CalcsTabViewModel`, `SkillsTabViewModel`, `LuaHost.GetActiveSkillsInGroup` + `GetMainActiveSkillIndex`):
- Single always-visible main-skill picker after the stats strip (★ + groups combo + active-skill sub-combo). Drives `mainSocketGroup` via existing `SetActiveSkillGroup` flow. Both selectors localise via `SkillGroupDisplayVm.DisplayName`.
- `BuildPageViewModel.CalcsTab` ctor accepts `onMainGroupChanged` callback → `SkillsTab.RefreshMainFlag()` + silent `AutoSaveAsync` (persists between sessions without explicit Save).
- `CalcsTabViewModel.RefreshSkillGroups` now picks the saved `GetMainSkillGroupIndex` group instead of `FirstOrDefault`.
- `RefreshActiveSkills` defaults to the first non-trigger active skill for CastOn / Meta groups (so opening a trigger group lands on the spell, not the trigger gem). Calls `SetActiveSkillGroup` to push the override into Lua so the engine recomputes DPS. `LuaHost.GetActiveSkillsInGroup` returns an `IsTrigger` flag (skillTypes[32] or [122]) emitted as `1/0` long to dodge NLua bool round-trip quirks.
- "(Главное)" text label removed from both top selector and Skills-tab group list — only the ★ remains.

**Bones of Ullr unique-edit bug** (`PBLEngine/LuaHost.cs`, `ItemEditorViewModel.cs`):
- Two separate bugs were dropping mods after edit + save on unique boots:
  1. *Wrong SelectedBase for uniques* — editor constructor seeded `Bases.FirstOrDefault()` ("Gold Amulet"), `ParseUniqueRaw` never realigned it. Soul Core of Citaqualotl then reported the WEAPON variant of its mod ("30% Elemental Damage with Attacks") instead of the ARMOUR variant ("+5% to all Elemental Resistances"). New `AlignBaseForUnique(baseName)` silently assigns SelectedBase (suppressing `OnBaseSelected` side effects) from both the unique-load path and the `OnSelectedUniqueChanged` handler. `GetRuneModsFor` generic fallback now picks `armour` first for armour-class slot types.
  2. *Variant-tagged mods filtered out post-save* — `BuildRawText` was emitting source-template `{variant:N}` prefixes without a matching `Selected Variant:` line; PoB's `CheckModLineVariant` then filtered most explicits out. `BuildRawLineForSave` now strips both `{range:X}` and `{variant:N}` markers. `UpdateItemFromText` Lua calls `newItem:UpdateRunes()` after parse to regenerate rune-implicit mods from `Rune:` lines (with `modList = modLib.parseMod(line) or {}` guard so downstream tooltip code doesn't crash on nil `.modList`).
- `GetItemTooltipLines` switched from `foreach (var k in tbl.Keys)` to a flat `\x1F`/`\t`-delimited Lua string + C# split. NLua's LuaTable.Keys enumerator silently drops sequence entries on long tooltip lists (36 lines became 24 reproducibly).

**Items-tab UI overhaul**:
- *50/50 resizable split* (`ItemsTabView`): main grid `*,5,*` with `GridSplitter`, left pane `MinWidth=540` to preserve the hardcoded 540×530 slot canvas.
- *Styled hover tooltip pipeline* — `Controls/StyledHoverTooltip.cs` attached property wires every slot and pool entry's hover from `HoverTooltipModel(Text, TitleColor)`. Builds a Border with inline `Run`s: rarity-coloured title + muted `(ilvl N)` tag + mod lines with numbers highlighted in `#F9E2AF` against cornflower-blue label text.
- *Right-pane tooltip rewrite* (`ItemTooltipView.axaml`, `ItemTooltipViewModel.Load`) — switched from `WrapPanel` of per-segment TextBlocks (which can't word-wrap a long line; only the row-of-runs wraps) to one `TextBlock` per line with inline Runs via new `InlinesHelper.ItemSegments` attached property. Long unique titles wrap inside the column instead of clipping. `ScaleSize` converter bumped 0.85× → 1.0× (denser scale was unreadable). New `LineHeightForSize` converter returns `size × 1.4` so big title glyphs ("Морьор Инвиктус") aren't clipped vertically by a too-tight default line box.
- *Equip / unequip delta block in its own column* — `Load` splits lines into `ItemLines` (block ≤ 1) and `DeltaLines` (block ≥ 2) with `HasDelta` flag. Right pane renders a Grid `*,Auto,*` with vertical divider between item info and delta block.
- *Spirit in the header strip* — `BuildModel.Spirit` (reads Lua `Spirit` stat), new `Sidebar_Spirit` resource key, AttrInt-coloured. Sits between Eva and the main-skill divider.
- *Removed equip-strip top row* from the editor pane (item name + compatible-slot dropdown + Equip/Unequip buttons). Functionality stays on pool entries themselves.

**Item editor polish**:
- *Sliders unclipped* — removed `MinWidth=200`, added 8 px right padding to the mods column and 4/8 padding around the slider Grid so the Avalonia Slider thumb has room to render past track ends.
- *Corrupted no longer locks edits* — `CanEditMods` now returns `true` unconditionally (planner semantics, not game semantics). Sliders, remove buttons and rune slots stay active on corrupted gear.
- *Editor typography bumped* — bulk inline `FontSize` raise: 10→12, 11→13, 12→14, 13→15. Section labels, action/save/cancel/rarity buttons switched to Inter at 13pt with extra padding for breathing room.
- *Runes in a dedicated right column* — `Grid ColumnDefinitions="*,Auto,Auto"` inside the editor body. Mods toolbar (+ Add Mod button + Prefix/Suffix indicator) and (collapsible) affix picker now sit at the top of the left column; runes column (260 px) starts at the same vertical level as the toolbar, not pushed below it.

**Corruption picker (dedicated pipeline)** (`LuaHost.GetItemCorruptedAffixes`, `ItemEditorViewModel.OpenCorruptionPickerCommand` + `AddCorruptionAffix`, `Editor_AddCorruption` / `Editor_CorruptionPickerHeader`):
- New Lua helper reads `data.itemMods.Corruption` (singular form was the empty-list bug — PoB's table is `Corruption`, not `Corrupted`) filtered by base item tags.
- Dedicated `IsCorruptionPickerOpen` flag + `FilteredCorruptionAffixes` collection + crimson collapsible Border below the mods toolbar. Mutually exclusive with the regular affix picker (opening one closes the other).
- "+ Добавить осквернение" red-bordered Button appears next to the Corrupted checkbox when the item is corrupted. Click → loads pool on demand → opens picker.
- Mods added through this pipeline carry `IsCorruption=true` on `ExplicitModViewModel`. Rendered with: red "C" badge, 3 px vivid-red left stripe, faint crimson row background (`#2A1015`). The full add-corruption pipeline (red button → red picker → red mod row) is visually self-contained.
...
Lua-воркеров (`PBLEngine/LuaWorkerPool`, ленивый прогрев при первом расчёте,
...
### Large features
- [x] **CalcsTab / stats window redesign** — done: domain-column color-coded collapsible cards + search filter + breakdown drawer (`593fead1e`), and a density toggle (compact 4-col masonry default vs large 3 domain columns, `ce11782f1`). ConfigTab later adopted the same compact masonry (`69a6ced66`).
...
- [x] Dark / light theme switch — done (Phase 11 entry). Follow-ups deferred: Trader hardcode cleanup, light tree canvas.
- [ ] **Кастомизация цветов пользователем** (уровень 3 плана «темизация», отложено 2026-07-12). Идея: новая секция в `AppSettingsWindow` (структура окна — вертикальный стек секций, задел уже есть) с редактированием избранных токенов из `Tokens.Colors.axaml` (акцент/бренд, фоны, семантические statcolors) поверх выбранного варианта Dark/Light. Реализация: пользовательские переопределения хранить в prefs (например, `ThemeOverrides` JSON: token→hex per variant), при старте и при изменении вливать поверх активного варианта через `ResourceDictionary` в `Application.Resources` (мгновенно, DynamicResource подхватит; C#-визуалы через `ThemeService.Brush` подхватят при следующем создании). Нужны: кнопка «Сбросить к стандартным», валидация контраста хотя бы предупреждением, и ограниченный whitelist токенов (не все ~60, а осмысленная поднабор-палитра). Вне scope: изменение always-dark групп (Tooltip*, HoverTip*, полотно дерева).
...
| Main-skill selector | Unified ★ + group combo + active-skill combo lives in the BuildPage header strip (next to stats). Binds to `CalcsTab.SkillGroups` / `SelectedSkillGroup` / `ActiveSkills` / `SelectedActiveSkill`. Changing the selector calls `SetActiveSkillGroup`, runs `RefreshMainFlag` on Skills, and triggers `AutoSaveAsync` so the choice persists between sessions. CastOn / trigger groups default to the first non-trigger active skill via `LuaHost.GetMainActiveSkillIndex` + `IsTrigger` flag from `GetActiveSkillsInGroup`. |
| Unique editor base sync | `ItemEditorViewModel.AlignBaseForUnique(baseName)` silently sets `SelectedBase` (with `_suppressBaseSelectedSideEffects = true`) without firing `OnBaseSelected`. Without this, `CurrentSlotType` resolves to the first alphabetical base ("Gold Amulet") and rune mods report the wrong slot variant. Called from both LoadEditingFromText (unique branch) and `OnSelectedUniqueChanged`. |
| Editor save raw flags | `ExplicitModViewModel.BuildRawLineForSave` strips BOTH `{range:X}` AND `{variant:N}` from the source-template raw line. Variant markers without a matching `Selected Variant:` line cause PoB's `CheckModLineVariant` to filter the mod out silently. Editor only knows the currently-displayed variant; emitting filtered raw is the right simplification. |
| UpdateItemFromText rune regen | After `new('Item', raw)`, call `newItem:UpdateRunes()` when `#newItem.runes > 0` — `ParseRaw` doesn't auto-regen rune-implicit mods from `Rune:` socket lines. Then iterate `runeModLines` setting `modList = modLib.parseMod(line) or {}` so `BuildModList` and `AddItemTooltip` don't crash on nil `modList` access. |
| Tooltip serialisation | `LuaHost.GetItemTooltipLines` returns a `\x1F` row-delimited / `\t` field-delimited string and splits on the C# side. NLua's `LuaTable.Keys` iterator silently drops sequence entries on long tables (36 rows → 24 reproducibly), so any new long-list crossings should use the same serialisation pattern. |
| Corruption picker key | PoB's data table is `data.itemMods.Corruption` (singular form), NOT `Corrupted`. `LuaHost.GetItemCorruptedAffixes(baseName)` filters by base tags exactly like `GetItemAffixes`. Picker UI lives in its own `IsCorruptionPickerOpen` collapsible Border (crimson border + dark crimson background), mutually exclusive with the regular affix picker. Mods added through it carry `ExplicitModViewModel.IsCorruption = true` for the red badge / accent stripe / row tint. |
| InlinesHelper variants | `ctl:InlinesHelper.Segments` (uses `TooltipTextSegment.Color`) for SkillsTab; `ctl:InlinesHelper.ItemSegments` (uses `TooltipSegment.ColorHex`) for ItemTooltipView. Both apply inline Runs to a single TextBlock so `TextWrapping=Wrap` actually wraps text — a `WrapPanel` of per-segment TextBlocks only wraps the row, not the text inside each Run. |
| Tooltip line height | `TooltipKindConverter.LineHeightForSize` returns `size × 1.4` (minimum 16). Always set `LineHeight={Binding Size, Converter=...}` on multi-size tooltip TextBlocks — a flat `LineHeight=22` value clipped 22-pt titles vertically (ascender/descender amputation). |
| StyledHoverTooltip | Slot / pool cells use `ctl:StyledHoverTooltip.Source="{Binding ...HoverTooltip}"` (binds `HoverTooltipModel(Text, TitleColor)`). The attached property builds a Border-wrapped panel with rarity-coloured title, muted ilvl tag, and number-highlighted mod lines (`#F9E2AF` numbers on `#8888FF` labels). Replaces plain `ToolTip.Tip` strings — never bind multi-line item info to a raw string Tip. |
